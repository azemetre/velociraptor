    ...
    render() {
        ...
        return (
            <Modal show={true} onHide={this.props.onCancel}>
                ...
                <Modal.Body>{T("RecursiveVFSMessage", path)}</Modal.Body>
                <Modal.Footer>
...
class VeloFileList extends Component {
    ...
    updateCurrentFile = (row) => {
        ...
        this.props.history.push("/vfs/" + this.props.client.client_id + EncodePathInURL(Join(vfs_path)));
    };
                ...
                specs: [
                    {
                        ...
                        parameters: {
                            env: [
                                { key: "Components", value: JSON.stringify(node.path.slice(1)) },
    ...
    render() {
        ...
        let toolbar = (
            <ButtonGroup className="float-right vfs-toolbar">
                {this.state.lastRefreshOperationId ? (
                    <Button
                        data-tooltip={T("Currently refreshing")}
                        data-position="right"
                    ...
                    <Button
                        data-tooltip={T("Refresh this directory (sync its listing with the client)")}
                        data-position="right"
                ...
                {this.state.lastRecursiveRefreshOperationId ? (
                    <Button
                        data-tooltip={T("Currently refreshing from the client")}
                        data-position="right"
                        className="btn-tooltip"
                    ...
                    >
                        ...
                        <span className="button-label">
                            Synced {(this.state.lastRecursiveRefreshData.total_collected_rows || 0) + " files"}
                        </span>
                        <span className="button-label">
                    ...
                    <Button
                        data-tooltip={T("Recursively refresh this directory (sync its listing with the client)")}
                        data-position="right"
                        className="btn-tooltip"
                        disabled={!this.props.node.path}
                ...
                {this.state.lastRecursiveDownloadOperationId ? (
                    <Button
                        data-tooltip={T("Currently fetching files from the client")}
                        data-position="right"
                        className="btn-tooltip"
                    ...
                    >
                        ...
                        <span className="button-label">
                            Downloaded{" "}
                            {(this.state.lastRecursiveDownloadData.total_uploaded_files || 0) +
                                " files (" +
                                (this.state.lastRecursiveDownloadData.total_uploaded_bytes / 1024 / 1024 || 0).toFixed(
                                    0
                                ) +
                                "Mb)"}
                        </span>
                    ...
                    <Button
                        data-tooltip={T("Recursively download this directory from the client")}
                        data-position="right"
                        className="btn-tooltip"
                ...
                <Link
                    to={"/collected/" + this.props.client.client_id + "/" + this.props.node.flow_id + "/overview"}
        ...
        if (!this.props.node || !this.props.node.flow_id) {
            return (
                <div className="fill-parent no-margins selectable col-12">
                    <Navbar className="toolbar">{toolbar}</Navbar>
        ...
        if (this.props.node.start_idx === this.props.node.end_idx) {
            return (
                <div className="fill-parent no-margins selectable col-12">
                    <Navbar className="toolbar">{toolbar}</Navbar>
                    <div className="fill-parent no-margins toolbar-margin">
                        <h5 className="no-content">{T("Directory is empty.")}</h5>
                    </div>
                </div>
            );
        }
        ...
        let formatters = {
            Download: (cell, row) => {
                let result = [];
                if (cell) {
                    result.push(<FontAwesomeIcon className="hint-icon" key="1" icon="save" />);
                }
                let fn_btime = row["_Data"] && row._Data["fn_btime"];
                let btime = row["btime"];
                if (btime < fn_btime) {
                    result.push(<FontAwesomeIcon key="2" className="file-stomped hint-icon" icon="clock" />);
                }
