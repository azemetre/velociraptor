...
import { InspectRawJson, ColumnToggleList, sizePerPageRenderer, PrepareData, formatColumns } from "./table.jsx";

const pageListRenderer = ({ pages, currentPage, totalRows, pageSize, onPageChange }) => {
    // just exclude <, <<, >>, >
    ...
    return (
        <Pagination>
            ...
            {pageWithoutIndication.map((p, idx) => (
                <Pagination.Item key={idx} active={p.active} onClick={() => onPageChange(p.page)}>
                    {p.page}
                </Pagination.Item>
            ))}
...
class VeloPagedTable extends Component {
    ...
    componentDidUpdate(prevProps, prevState, snapshot) {
        ...
        if (this.props.transform && !_.isEqual(prevProps.transform, this.props.transform)) {
    ...
    fetchRows = () => {
        ...
        api.get(url, params, this.source.token)
            .then((response) => {
                if (response.cancel) {
                    return;
                }
    ...
    customTotal = (from, to, size) => (
        <span className="react-bootstrap-table-pagination-total">{T("TablePagination", from, to, size)}</span>
    );

    render() {
        let timezone = (this.context.traits && this.context.traits.timezone) || "UTC";
                ...
                return (
                    <>
                        <div className="col-12">
                            <Navbar className="toolbar">{this.props.toolbar || <></>}</Navbar>
        ...
        for (var i = 0; i < this.state.columns.length; i++) {
            ...
            let definition = { dataField: name, text: this.props.translate_column_headers ? T(name) : name };
            if (this.props.renderers && this.props.renderers[name]) {
                definition.formatter = this.props.renderers[name];
            } else {
                definition.formatter = this.getColumnRenderer(name, this.state.column_types);
            }
                        ...
                        <div className="col-12">
                            <Navbar className="toolbar">
                                ...
                                {transformed.length > 0 && (
                                    <ButtonGroup className="float-right">{transformed}</ButtonGroup>
                                )}
                                {this.props.toolbar || <></>}
                            </Navbar>
