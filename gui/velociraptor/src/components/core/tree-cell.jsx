...
class TreeCellDialog extends Component {
    ...
    render() {
        ...
        return (
            ...
            >
                <Modal.Header closeButton>{T(this.props.name)}</Modal.Header>
                <Modal.Body className="tree-modal-body">
                    <SplitPane split="vertical" defaultSize="40%" resizerStyle={resizerStyle}>
                        <TreeView data={data} onSelect={this.onToggle} />
                        <div className="tree-data-pane">
                            {value ? <VeloValueRenderer value={value} /> : <div>No Data</div>}
                        </div>
                    </SplitPane>
                </Modal.Body>
                <Modal.Footer>
        ...
        return (
            <div>
                ...
                {this.props.data && this.props.data.name && (
                    <Button onClick={() => this.setState({ showDialog: true })} variant="default">
