...
export default class VeloAce extends Component {
    ...
    getUserOptions = () => {
        let user_options = this.normalizeOptions(JSON.parse(this.context.traits.ui_settings || "{}"));
    ...
    updatePreferences = (e, editor) => {
        ...
        if (!_.isEqual(new_options, this.getUserOptions())) {
            api.post("v1/SetGUIOptions", { options: JSON.stringify(new_options) }, this.source.token).then(
                (response) => {
    ...
    render() {
        ...
        return (
            <>
                <div className={classNames("col-12", "velo-ace-editor", this.props.className)}>
                    <AceEditor
                        ref="ace"
