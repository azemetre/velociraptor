        ...
        return (
            <>
                ...
                >
                    ...
                    <Modal.Body>
                        <VeloAce text={serialized} mode="json" aceConfig={this.aceConfig} />
                    </Modal.Body>
...
export function headerFormatter(column, colIndex, { sortElement, filterElement }) {
    let result = (
        ...
        <table className="notebook-filter">
            <tbody>
                <tr>
                    {column.filter ? <td>{filterElement}</td> : <td>{column.text}</td>}
...
export function sortCaret(order, column) {
    ...
    else if (order === "asc") return <FontAwesomeIcon icon="sort-up" />;
    else if (order === "desc") return <FontAwesomeIcon icon="sort-down" />;
    ...
    _.each(columns, (x) => {
        ...
        switch (x.type) {
            ...
            case "tree":
                x.formatter = (cell, row) => {
                    if (_.isObject(cell)) {
                        return <TreeCell name={x.text} data={cell} />;
                    }
                    return cell;
                };
                x.type = null;
            ...
            case "flow":
                x.formatter = (cell, row) => {
                    ...
                    return (
                        <NavLink tabIndex="0" id={cell} to={"/collected/" + client_id + "/" + cell}>
                            {cell}
                        </NavLink>
                    );
                };
            ...
            case "preview_upload":
                x.formatter = (cell, row) => {
                    ...
                    return <PreviewUpload env={env} upload={cell} />;
                };
                x.type = null;
                break;
