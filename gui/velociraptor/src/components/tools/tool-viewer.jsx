...
class ResetToolDialog extends React.Component {
    ...
    setToolInfo = (tool) => {
        api.post("v1/SetToolInfo", tool, this.source.token)
            .then((response) => {
    ...
    render() {
        return (
            <Modal show={true} enforceFocus={true} scrollable={false} onHide={this.props.onClose}>
                <Modal.Header closeButton>
                    <Modal.Title>
                        {T("Tool")} {this.props.tool && this.props.tool.name}
                    </Modal.Title>
                </Modal.Header>
                <Modal.Body className="tool-viewer">
                    ...
                    <Button onClick={(x) => this.setToolInfo(this.props.tool)} variant="outline-info">
...
export default class ToolViewer extends React.Component {
    ...
    componentDidUpdate = (prevProps, prevState, rootNode) => {
        if (this.props.name !== prevProps.name || this.props.version !== prevProps.version) {
            this.fetchToolInfo();
        }
    };

    fetchToolInfo = (onclose) => {
        api.get("v1/GetToolInfo", { name: this.props.name }, this.source.token)
            .then((response) => {
                this.setState({ tool: response.data });
            })
            .then(onclose);
    ...
    uploadFile = () => {
        ...
        api.upload("v1/UploadTool", { file: this.state.tool_file }, this.state.tool).then((response) => {
    ...
    render() {
            ...
            cards.push(
                <Card key={1}>
                    <Card.Header>
                        {T("Served Locally")}
                        <span
                            className="float-right clickable close-icon"
                    ...
                    <Card.Body>
                        <Card.Text>{T("ToolLocalDesc")}</Card.Text>
                        {tool.url && (
                            <Button disabled={this.state.inflight} onClick={this.serve_upstream}>
        ...
        if (!tool.serve_locally && !this.state.hide_2) {
            cards.push(
                <Card key={2}>
                    ...
                    <Card.Body>
                        <Card.Text>{T("ServedFromURL", api.base_path, tool.url)}</Card.Text>
                        {tool.url && (
                            <Button disabled={this.state.inflight} onClick={this.serve_locally}>
                                {T("Serve Locally")}
                            </Button>
                        )}
                    </Card.Body>
        ...
        if (tool.github_project && !this.state.hide_3) {
            cards.push(
                <Card key={3}>
                    ...
                    <Card.Body>
                        <Card.Text>{T("ServedFromGithub", tool.github_project, tool.github_asset_regex)}</Card.Text>
                        <Button variant="primary" disabled={this.state.inflight} onClick={this.redownloadFile}>
                            {T("Refresh Github")}
                        </Button>
                    </Card.Body>
                </Card>
        ...
        if (!tool.hash && !this.state.hide_4) {
            cards.push(
                <Card key={4}>
                    ...
                    <Card.Body>
                        <Card.Text>{T("PlaceHolder")}</Card.Text>
                        <Button variant="primary" disabled={this.state.inflight} onClick={this.calculateHash}>
                            {T("Materialize Hash")}
                        </Button>
                    </Card.Body>
                </Card>
        ...
        if (tool.hash && !this.state.hide_5) {
            cards.push(
                <Card key={5}>
                    ...
                    <Card.Body>
                        <Card.Text>{T("ToolHash")}</Card.Text>
                        <Button variant="primary" disabled={this.state.inflight} onClick={this.redownloadFile}>
                            {T("Re-Download File")}
                        </Button>
                    </Card.Body>
        ...
        if (tool.admin_override && !this.state.hide_6) {
            cards.push(
                <Card key={6}>
                    ...
                    <Card.Body>
                        <Card.Text>{T("AdminOverride")}</Card.Text>
                    </Card.Body>
                </Card>
            );
        }
        ...
        if (!tool.hash && !this.state.url && !this.state.github_project) {
            cards.push(
                <Card key={7}>
                    ...
                    <Card.Body>
                        <Card.Text>{T("ToolError")}</Card.Text>
                    </Card.Body>
                </Card>
            );
        }
        ...
        let tool_version_options = _.map(tool.versions, (x) => {
            return { value: x.artifact, tool: x, label: x.artifact, isFixed: true };
        });
        return (
            <>
                {this.state.showUpdateDialog && (
                    <ResetToolDialog
                        ...
                        onClose={(x) => this.fetchToolInfo((x) => this.setState({ showUpdateDialog: false }))}
                    ></ResetToolDialog>
                )}
                <Modal
                    ...
                    <Modal.Body className="tool-viewer">
                        ...
                        <CardDeck>
                            <Card>
                                ...
                                <Card.Body>
                                    <Card.Text>{T("OverrideToolDesc")}</Card.Text>
                                    ...
                                    <Form className="selectable">
                                        <InputGroup>
                                            ...
                                            <Form.Control
                                                ...
                                                onChange={(e) => this.setState({ remote_url: e.currentTarget.value })}
                                            />
                                        </InputGroup>
                                    </Form>
                                </Card.Body>
