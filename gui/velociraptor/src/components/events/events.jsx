...
class InspectRawJson extends React.PureComponent {
    ...
    fetchEventTable = () => {
        ...
        // "server".
        let client_id = (this.props.client && this.props.client.client_id) || "server";
        if (client_id === "server") {
...
class EventMonitoring extends React.Component {
    ...
    componentDidUpdate = (prevProps, prevState, rootNode) => {
        let client_id = (this.props.client && this.props.client.client_id) || "server";
        let prev_client_id = (prevProps.client && prevProps.client.client_id) || "server";
    ...
    fetchEventResults = () => {
        ...
        api.post(
            ...
            let router_artifact = this.props.match && this.props.match.params && this.props.match.params.artifact;
            if (router_artifact) {
    ...
    setEventTable = (request) => {
        api.post("v1/SetClientMonitoringState", request, this.source.token).then((resp) => {
            this.setState({ showEventTableWizard: false });
        });
    };

    setServerEventTable = (request) => {
        api.post("v1/SetServerMonitoringState", request, this.source.token).then((resp) => {
    ...
    render() {
        ...
        let column_types =
            this.state.artifact && this.state.artifact.definition && this.state.artifact.definition.column_types;
        ...
        return (
            <>
                ...
                <Navbar className="artifact-toolbar justify-content-between">
                    <ButtonGroup>
                        {client_id === "server" || client_id === "" ? (
                            <>
                                <Button
                                    data-tooltip={T("Update server monitoring table")}
                                    data-position="right"
                                ...
                                <Button
                                    data-tooltip={T("Show server monitoring tables")}
                                    data-position="right"
                                    className="btn-tooltip"
                            ...
                            <>
                                <Button
                                    data-tooltip={T("Update client monitoring table")}
                                    data-position="right"
                                    className="btn-tooltip"
                                ...
                                <Button
                                    data-tooltip={T("Show client monitoring tables")}
                                    data-position="right"
                                    className="btn-tooltip"
                                    onClick={() => this.setState({ showEventMonitoringPopup: true })}
                                    variant="default"
                        ...
                        <Dropdown
                            data-tooltip={this.state.artifact.artifact || T("Select artifact")}
                            data-position="right"
                            className="btn-tooltip"
                            variant="default"
                        >
                            ...
                            <Dropdown.Menu>
                                {_.map(this.state.available_artifacts, (x, idx) => {
                                    let active_artifact = this.state.artifact && this.state.artifact.artifact;
                                    return (
                                        <Dropdown.Item
                                            key={idx}
                                            title={x.artifact}
                ...
                {(this.state.mode === mode_raw_data || this.state.mode === mode_logs) &&
                ...
                {this.state.showDeleteNotebook && (
                    <DeleteNotebookDialog
                        notebook_id={get_notebook_id(this.state.artifact.artifact, client_id)}
                        onClose={(e) => {
