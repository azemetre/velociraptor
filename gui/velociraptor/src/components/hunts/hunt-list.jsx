...
class HuntList extends React.Component {
    ...
    componentDidMount = () => {
        ...
        let action = this.props.match && this.props.match.params && this.props.match.params.hunt_id;
        if (action === "new") {
    ...
    startHunt = () => {
        let hunt_id = this.props.selected_hunt && this.props.selected_hunt.hunt_id;
    ...
    setFullScreen = () => {
        if (this.props.selected_hunt) {
            this.props.history.push("/fullscreen/hunts/" + this.props.selected_hunt.hunt_id + "/notebook");
        }
    };
        ...
        return (
            <>
                ...
                {this.state.showRunHuntDialog && (
                    <Modal
                        show={this.state.showRunHuntDialog}
                    ...
                    >
                        ...
                        <Modal.Footer>
                            <Button variant="secondary" onClick={() => this.setState({ showRunHuntDialog: false })}>
                                {T("Close")}
                            </Button>
                            <Button variant="primary" onClick={this.startHunt}>
                                {T("Run it!")}
                            </Button>
                        </Modal.Footer>
                    </Modal>
                ...
                {this.state.showDeleteHuntDialog && (
                    <Modal show={true} onHide={() => this.setState({ showDeleteHuntDialog: false })}>
                        <Modal.Header closeButton>
                            <Modal.Title>{T("Permanently delete this hunt?")}</Modal.Title>
                        </Modal.Header>

                        <Modal.Body>{T("DeleteHuntDialog")}</Modal.Body>

                        <Modal.Footer>
                            <Button variant="secondary" onClick={() => this.setState({ showDeleteHuntDialog: false })}>
                                {T("Close")}
                            </Button>
                            <Button variant="primary" onClick={this.deleteHunt}>
                                {T("Kill it!")}
                            </Button>
                        </Modal.Footer>
                    </Modal>
    ...
    return formatColumns([
        {
            ...
            formatter: (cell, row) => {
                ...
                if (stopped || cell === "STOPPED") {
                    return (
                        <div className="hunt-status-icon">
                            <FontAwesomeIcon icon="stop" />
                        </div>
                    );
                }
