...
export default class HuntOverview extends React.Component {
    ...
    prepareDownload = (download_type) => {
        let lock_password = "";
        if (this.state.lock) {
            lock_password = this.context.traits && this.context.traits.default_password;
        }
        ...
        api.post("v1/CreateDownload", params, this.source.token).then((resp) => {
                ...
                <Card>
                    ...
                    <Card.Body>
                        <dl className="row">
                            ...
                            <dd className="col-8">
                                <VeloTimestamp usec={hunt.create_time / 1000} />
                            </dd>

                            <dt className="col-4">{T("Expiry Time")}</dt>
                            <dd className="col-8">
                                <VeloTimestamp usec={hunt.expires / 1000} />
                            </dd>
                            ...
                            <dd className="col-8">{start_request.ops_per_second || T("Unlimited")}</dd>
                            <dt className="col-4">{T("CPU Limit")}</dt>
                            <dd className="col-8">{start_request.cpu_limit || T("Unlimited")}</dd>
                            <dt className="col-4">{T("IOPS Limit")}</dt>
                            <dd className="col-8">{start_request.iops_limit || T("Unlimited")}</dd>
                            {labels && (
                                <>
                ...
                <Card>
                    ...
                    <Card.Body>
                        <dl className="row">
                            ...
                            <dd className="col-8">{stats.total_clients_scheduled}</dd>
                            ...
                            <dd className="col-8">
                                <ButtonGroup>
                                    ...
                                    <Dropdown>
                                        <Dropdown.Toggle disabled={this.state.preparing} variant="default">
                                            <FontAwesomeIcon icon="archive" />
                                        </Dropdown.Toggle>
                                        <Dropdown.Menu>
                                            <Dropdown.Item onClick={() => this.prepareDownload("all")}>
                                                {T("Full Download")}
                                            </Dropdown.Item>
                                            <Dropdown.Item onClick={() => this.prepareDownload("summary")}>
                                                {T("Summary Download")}
                                            </Dropdown.Item>
                                            <Dropdown.Item onClick={() => this.prepareDownload("summary-csv")}>
                                                {T("Summary (CSV Only)")}
                                            </Dropdown.Item>
                                            <Dropdown.Item onClick={() => this.prepareDownload("summary-json")}>
                                                {T("Summary (JSON Only)")}
                                            </Dropdown.Item>
                                        </Dropdown.Menu>
                                    </Dropdown>
