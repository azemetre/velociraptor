...
class HexViewTab extends React.PureComponent {
    ...
    componentDidUpdate = (prevProps, prevState, rootNode) => {
        if (!_.isEqual(prevProps.params, this.props.params) || !_.isEqual(prevState.page, this.state.page)) {
            this.fetchPage_();
        }
    };
    ...
    parseFileContentToTextRepresentation_ = (intArray) => {
        ...
        for (var i = 0; i < intArray.length; i++) {
            ...
            if ((c >= 0x20 && c < 0x7f) || c === 10 || c === 13 || c === 9) {
                    ...
                    <Tabs activeKey={this.state.tab} onSelect={(tab) => this.setState({ tab: tab })}>
                        <Tab eventKey="overview" title={T("Overview")}>
                            {this.state.tab === "overview" && (
                                <HexViewTab params={this.props.params} url={this.props.url} size={this.props.size} />
                            )}
                        </Tab>
                        <Tab eventKey="details" title={T("Details")}>
                            {this.state.tab === "details" && <VeloValueRenderer value={this.props.upload} />}
                        </Tab>
                    </Tabs>
                </Modal.Body>
                <Modal.Footer>
...
export default class PreviewUpload extends Component {
    ...
    fetchPreview_ = () => {
        ...
        let params = {
            ...
            fs_components: normalizeComponentList(components, client_id, flow_id, accessor),
            client_id: client_id,
    ...
    render() {
        ...
        if (checkMime(this.state.view)) {
            ...
            let url = api.base_path + "/api/" + this.state.url + "?" + qs.stringify(params, { indices: false });
            ...
            <>
                {this.state.showDialog && this.state.params && (
                    <InspectDialog
                        ...
                        url={this.state.url}
                        size={this.props.upload.StoredSize || this.props.upload.Size || 0}
                        upload={this.props.upload}
                        onClose={() => this.setState({ showDialog: false })}
                    />
                )}
                <Button
                    className="hex-popup client-link"
                    size="sm"
...
const normalizeComponentList = (components, client_id, flow_id, accessor) => {
    ...
    return ["clients", client_id, "collections", flow_id, "uploads", accessor].concat(components);
};
