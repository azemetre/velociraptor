    ...
    render() {
        return (
            <div className="timeline-value-container">
                {this.state.expanded ? (
                    <span>
                        <Button variant="default-outline" size="sm" onClick={() => this.setState({ expanded: false })}>
                    ...
                    <span>
                        <Button variant="default-outline" size="sm" onClick={() => this.setState({ expanded: true })}>
                            <FontAwesomeIcon icon="plus" />
                        </Button>
                        {_.map(this.props.value, (v, k) => {
                            let value = JSON.stringify(v);
                            return (
                                <span key={k} className="timeline-value-item">
                                    {k}: {value}
                                </span>
                            );
                        })}
        ...
        let columns = [
            { dataField: "_id", hidden: true },
            {
                ...
                formatter: (cell, row, rowIndex) => {
                    return (
                        <div className={this.getTimelineClass(_.toString(row._Source))}>
    ...
    fetchRows = () => {
        ...
        api.get(url, params, this.source.token).then((response) => {
            ...
            if (this.state.visibleTimeStart === 0) {
                ...
                this.setState({ visibleTimeStart: visibleTimeStart, visibleTimeEnd: visibleTimeEnd });
            }
        });
    };
    ...
    render() {
        ...
        return (
            <div className="super-timeline">
                Super-timeline {this.props.name}
                    ...
                    <TimelineMarkers>
                        <CustomMarker date={this.state.start_time}>
                            {({ styles, date }) => {
                                ...
                                return <div style={styles} className="timeline-marker" />;
                            }}
                        </CustomMarker>
                    </TimelineMarkers>
                </Timeline>
                {this.state.columns && <TimelineTableRenderer timelines={super_timeline} rows={this.state.rows} />}
            </div>
        );
    }
}
