...
export default class UploadFileForm extends Component {
    ...
    uploadFile = () => {
        ...
        api.upload("v1/UploadFormFile", { file: this.state.upload }, this.props.param)
            .then((response) => {
                let url = response.data.url;
                this.props.setValue(url);

                this.setState({ loading: false, upload: {}, upload_info: response.data });
            })
            .catch((response) => {
        ...
        return (
            <Form.Group as={Row}>
                <Form.Label column sm="3">
                    <OverlayTrigger delay={{ show: 250, hide: 400 }} overlay={(props) => renderToolTip(props, param)}>
                        <div>{name}</div>
                    </OverlayTrigger>
                </Form.Label>
                <Col sm="8">
                    <InputGroup className="mb-3">
                        <InputGroup.Prepend>
                            ...
                            >
                                {this.state.loading ? <FontAwesomeIcon icon="spinner" spin /> : T("Upload")}
                            </InputGroup.Text>
                        </InputGroup.Prepend>
                        <Form.File custom>
                            <Form.File.Input
                            ...
                            {this.state.upload_info.filename ? (
                                <Form.File.Label data-browse="Select a different file">
                                    <a href={api.href(this.state.upload_info.url)}>{this.state.upload_info.filename}</a>
                                </Form.File.Label>
                            ) : (
                                <Form.File.Label data-browse="Select file">
                                    {this.state.upload.name ? this.state.upload.name : T("Click to upload file")}
                                </Form.File.Label>
                            )}
                        </Form.File>
                    </InputGroup>
