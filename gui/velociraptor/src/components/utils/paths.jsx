...
const PathJoin = function (path, component) {
    ...
    if (component.includes("/") || component[0] === '"' || component.includes("\\")) {
...
const ConsumeComponent = function (path) {
    ...
    if (path[0] === '"') {
        ...
        for (var i = 1; i < path.length; i++) {
            if (path[i] === '"') {
                ...
                if (next_char === '"') {
                    ...
                    return { next_path: path.substr(i + 1, path.length), component: result };
                } else {
        ...
        for (var j = 0; j < path.length; j++) {
            if (path[j] === "/" || path[i] === "\\") {
                return { next_path: path.substr(j, path.length), component: path.substr(0, j) };
            }
        }
    }
