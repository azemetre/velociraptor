...
const renderToolTip = (props, ts) => {
    ...
    return <Tooltip {...props}>{T("HumanizeDuration", difference)}</Tooltip>;
};
...
export const ToStandardTime = (value) => {
    ...
    if (!_.isNaN(value) && _.isNumber(value) && value > 0) {
        // Maybe nsec
        if (value > 20000000000000000) {
            // 11 October 2603 in microsec
            value /= 1000000; // To msec
        } else if (value > 20000000000000) {
            // 11 October 2603 in milliseconds
            value /= 1000;
        } else if (value > 20000000000) {
            // 11 October 2603 in seconds
            // Already in msec.
        } else {
...
class VeloTimestamp extends Component {
    ...
    render() {
        ...
        var when = moment(ts);
        return (
            <OverlayTrigger delay={{ show: 250, hide: 400 }} overlay={(props) => renderToolTip(props, ts)}>
                <div className="timestamp">{moment.tz(when, timezone).format()}</div>
            </OverlayTrigger>
        );
    }
}
