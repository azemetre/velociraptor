...
const Spanish = {
    ...
    "Quarantine description": (
        <>
            ...
            <p>
                Durante la cuarentena, el host no podrá comunicarse con ninguna otra red, salvo con el servidor de
                Velociraptor.
            </p>
        </>
    ),
    ...
    "Cannot Quarantine host message": (os_name, quarantine_artifact) => (
        <>
            <Alert variant="warning">
                {quarantine_artifact ? (
                    <p>
                        Esta instancia de Velociraptor no tiene el artefacto <b>{quarantine_artifact}</b> necesario para
                        poner en cuarentena los hosts que ejecutan {os_name}.
                    </p>
                ) : (
                    <p>
                        Esta instancia de Velociraptor no tiene un nombre de artefacto definido para poner en cuarentena
                        los hosts que ejecutan {os_name}.
                    </p>
                )}
            </Alert>
        </>
    ...
    "Select artifact pack (Zip file with YAML definitions)":
        "Seleccionar paquete de artefactos (archivo zip con definiciones YAML)",
    ...
    "Set a password in user preferences to lock the download file.":
        "Establecer una contraseña en las preferencias del usuario para bloquear el archivo de descarga.",
    ...
    "Please create a hunt that collects one or more of the following artifacts.":
        "Por favor, cree un Hunt que recopile uno o más de los siguientes artefactos.",
    Requests: "Peticiones",
    ...
    ArtifactDeletionDialog: (session_id, artifacts, total_bytes, total_rows) => (
        <>
            ...
            <b>{session_id}</b>.
            <br />
            Esta colección comprende los artefactos <b className="wrapped-text">{artifacts}</b>
            <br />
            <br />
            Se espera liberar {total_bytes.toFixed(0)} MB de datos y {total_rows} filas.
        </>
    ),
    "Save this collection to your Favorites": "Guardar esta colección en favoritos",
    ArtifactFavorites: (artifacts) => (
        <>
            Puede recuperar fácilmente la misma colección de los favoritos en el futuro.
            <br />
    ...
    "No artifacts configured. Please add some artifacts to collect":
        "No hay artefactos configurados. Agregue algunos artefactos para recopilar",
    ...
    "If set collection will be terminated after this many seconds with no progress.":
        "Si se encuentra configurado, la recopilación finalizará después de estos segundos sin que haya habido progreso.",
    ...
    "Create Offline collector: Select artifacts to collect":
        "Crear recopilador sin conexión: seleccionar artefactos para recopilar",
    "Configure Collection": "Configurar Colección",
    "Create Offline Collector: Configure artifact parameters":
        "Crear recopilador sin conexión: configurar parámetros de artefactos",
    ...
    "No hunts exist in the system. You can start a new hunt by clicking the New Hunt button above.":
        "No hay búsquedas en el sistema. Puede iniciar una nueva búsqueda haciendo clic en el botón Nueva búsqueda. ",
    ...
    RecursiveVFSMessage: (path) => (
        <>
            ...
            <br />
            <br />
            Esto permite que se transfieran grandes cantidades de datos desde el equipo final. El límite de carga
            predeterminado es de 1 GB, pero puede cambiarlo en la pantalla Artefactos recopilados.
        </>
    ),

    Textview: "Vista en texto plano",
    HexView: "Vista en Hexadecimal",
    "Refresh this directory (sync its listing with the client)":
        "Actualizar este directorio (sincronizar su listado con el host)",
    "Recursively refresh this directory (sync its listing with the client)":
        "Actualizar recursivamente este directorio (sincronizar su listado con el host)",
    ...
    "No data available. Refresh directory from client by clicking above.":
        "No hay datos disponibles. Actualice el directorio del host haciendo clic arriba.",
    "Please select a file or a folder to see its details here.":
        "Seleccione un archivo o una carpeta para ver sus detalles aquí.",
    ...
    ToolLocalDesc: (
        <>
            La herramienta será proporcionada por el servidor de Velociraptor a los hosts si es necesario. El host
            almacenará en caché la herramienta en su propio disco y comparará el hash la próxima vez si es necesario.
            Las herramientas solo se descargarán si su hash ha cambiado.
        </>
    ),
    ServedFromURL: (base_path, url) => (
        <>
            Los hosts descargarán la herramienta directamente de
            <a href={base_path + url}>{url}</a> si es necesario. Tenga en cuenta que si el hash no coincide con el hash
            esperado, los hosts rechazarán el archivo.
        </>
    ),
    ServedFromGithub: (github_project, github_asset_regex) => (
        <>
            La URL de la herramienta será actualizada por GitHub como la última versión del proyecto
    ...
    PlaceHolder: (
        <>
            El hash de la herramienta es desconocido. La primera vez que se necesite la herramienta, Velociraptor la
            descargará desde la URL y calculará su hash.
        </>
    ),
    ToolHash: (
        <>
            Se calculó el hash de la herramienta. Cuando los hosts necesiten usar esta herramienta se asegurarán de que
            este hash coincida con lo que se descarga.
        </>
    ),
    AdminOverride: (
        <>
            La herramienta fue subida manualmente por un administrador - no será actualiza automáticamente con la
            próxima actualización del servidor Velociraptor.
        </>
    ),
    ToolError: (
        <>
            Se desconoce el hash de la herramienta y no se ha definido una URL. Será imposible emplear esta herramienta
            en un Artefacto ya que Velociraptor no puede resolverlo. Como alternativa, puede cargar un archivo
            manualmente.
        </>
    ),
    OverrideToolDesc: (
        <>
            Como administrador, puede cargar manualmente un archivo ejecutable para ser utilizado como herramienta. Esto
            sobreescribirá la configuración de URL, y proporcionará la herramienta a todos aquellos Artefactos que lo
            necesitan. Como alternativa, puede configurar una URL para que los hosts puedan obtener herramientas.
        </>
    ),
    ...
    "Select label to edit its event monitoring table":
        "Seleccione una etiqueta para editar su tabla de monitorización de eventos",
    EventMonitoringCard: (
        <>
            La monitorización de eventos se enfoca a grupos de etiquetas específicos. Seleccione un grupo de etiquetas
            para configurar artefactos de eventos específicos dirigidos a este grupo.
        </>
    ),
    ...
    "Event Monitoring: Select artifacts to collect from label group ":
        "Monitorización de eventos: seleccione artefactos para recopilar del grupo de etiquetas ",
    "Artifact Collected": "Artefacto Recolectado",
    "Event Monitoring: Configure artifact parameters for label group ":
        "Monitorización de eventos: configurar parámetros de artefactos para el grupo de etiquetas ",
    ...
    "Server Event Monitoring: Select artifacts to collect on the server":
        "Monitorización de eventos del servidor: seleccione los artefactos para recopilar en el servidor",
    "Server Event Monitoring: Configure artifact parameters for server":
        "Monitorización de eventos del servidor: Configurar parámetros de artefactos para el servidor",
    "Server Event Monitoring: Review new event tables":
        "Monitorización de eventos del servidor: revisar nuevas tablas de eventos",
    ...
    "You are about to permanently delete the notebook for this hunt":
        "Está a punto de eliminar de forma permanente el bloc de notas correspondiente a este Hunt",
