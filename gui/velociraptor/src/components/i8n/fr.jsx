...
const French = {
    ...
    "Quarantine description": (
        <>
            ...
            <p>
                Pendant la quarantaine, l'hôte ne pourra pas communiquer avec tous les autres réseaux sauf avec le
    ...
    "Cannot Quarantine host message": (os_name, quarantine_artifact) => (
        <>
            <Alert variant="warning">
                {quarantine_artifact ? (
                    <p>
                        Cette instance Velociraptor n'a pas l'artefact <b>{quarantine_artifact}</b> requis pour mettre
                        en quarantaine les hôtes en cours d'exécution {os_name}.
                    </p>
                ) : (
                    <p>
                        Cette instance Velociraptor n'a pas de nom d'artefact défini pour mettre en quarantaine les
                        hôtes en cours d'exécution {os_name}.
                    </p>
                )}
            </Alert>
        </>
    ...
    "Select artifact pack (Zip file with YAML definitions)":
        "Sélectionner le paquet d'artefact (fichier zip avec YAML)",
    ...
    "Please create a hunt that collects one or more of the following artifacts.":
        "Veuillez créer une chasse qui collecte un ou plusieurs des artefacts suivants.",
    Requests: "Demandes",
    ...
    ArtifactDeletionDialog: (session_id, artifacts, total_bytes, total_rows) => (
        <>
            ...
            <b>{session_id}</b>.
            <br />
            Cette collection avait les artefacts <b className="wrapped-text">{artifacts}</b>
            <br />
            <br />
            Nous supposons de libérer {total_bytes.toFixed(0)} Mo de stockage de données et {total_rows} lignes.
        </>
    ),
    "Save this collection to your Favorites": "Enregistrer cette collection dans vos favoris",
    ArtifactFavorites: (artifacts) => (
        <>
            Vous pouvez facilement collecter la même collection de vos favoris à l'avenir.
            <br />
    ...
    "No artifacts configured. Please add some artifacts to collect":
        "Aucun artefact configuré. Veuillez ajouter quelques artefacts à collecter",
    ...
    "If set collection will be terminated after this many seconds with no progress.":
        "Si l'ensemble de collecte sera terminé après ce nombre de secondes sans progression.",
    ...
    "Create Offline collector: Select artifacts to collect":
        "Créer un collecteur hors ligne: sélectionnez les artefacts à collecter",
    "Configure Collection": "Configurer la collecte",
    "Create Offline Collector: Configure artifact parameters":
        "Créer un collecteur hors ligne: configurer les paramètres d'artefact",
    ...
    "No hunts exist in the system. You can start a new hunt by clicking the New Hunt button above.":
        "Aucune chasse n'existe dans le système. Vous pouvez commencer une nouvelle chasse en cliquant sur le bouton Nouvelle chasse ci-dessus.",
    ...
    RecursiveVFSMessage: (path) => (
        <>
            ...
            <br />
            <br />
            Cela permet de transférer de grandes quantités de données depuis le terminal. La limite de téléchargement
            par défaut est de 1 Go, mais vous pouvez la modifier dans l'écran des artefacts collectés.
        </>
    ),
    ...
    "Refresh this directory (sync its listing with the client)":
        "Mettre à jour ce répertoire (synchroniser son entrée) avec le client",
    "Recursively refresh this directory (sync its listing with the client)":
        "Actualisez récursivement ce répertoire (synchronisez sa liste avec le client)",
    ...
    "No data available. Refresh directory from client by clicking above.":
        "Pas de données disponibles. Actualiser le répertoire du client en cliquant ci-dessus.",
    ...
    ToolLocalDesc: (
        <>
            L'outil sera servi du serveur Velociraptor aux clients si nécessaire. Le client mettra l'outil en cache sur
            son propre disque et comparera le hachage la prochaine fois qu'il en aura besoin. Les outils ne seront
            téléchargés que si leur hachage a changé.
        </>
    ),
    ServedFromURL: (base_path, url) => (
        <>
            Les clients iront chercher l'outil directement à partir de
            <a href={base_path + url}>{url}</a> si nécessaire. Notez que si le hachage ne correspond pas au hachage
            attendu, les clients rejetteront le fichier.
        </>
    ),
    ServedFromGithub: (github_project, github_asset_regex) => (
        <>
            L'URL de l'outil est mise à jour par GitHub comme dernière version du projet
    ...
    PlaceHolder: (
        <>
            Le hachage de l'outil est actuellement inconnu. La première fois que l'outil est nécessaire, Velociraptor le
            télécharge à partir de son URL en amont et calcule son hachage.
        </>
    ),
    ToolHash: (
        <>
            Le hachage de l'outil a été calculé. Lorsque les clients ont besoin d'utiliser cet outil, ils s'assurent que
            ce hachage correspond à ce qu'ils téléchargent.
        </>
    ),
    AdminOverride: (
        <>
            L'outil a été téléchargé manuellement par un administrateur - il ne sera pas automatiquement mis à jour lors
            de la prochaine mise à jour du serveur Velociraptor
        </>
    ),
    ToolError: (
        <>
            Le hachage de l'outil n'est pas connu et aucune URL n'est définie. Il sera impossible d'utiliser cet outil
            dans un artefact car Velociraptor est incapable de le résoudre. Vous pouvez télécharger manuellement un
            fichier.
        </>
    ),
    OverrideToolDesc: (
        <>
            En tant qu'administrateur, vous pouvez télécharger manuellement un fichier binaire à utiliser comme outil.
            Cela remplacera le paramètre d'URL en amont et fournira votre outil à tous les artefacts qui en ont besoin.
            Alternative, définissez une URL pour que les clients récupèrent les outils.
        </>
    ),
    ...
    "Select label to edit its event monitoring table":
        "Sélectionnez le libellé pour modifier sa table de surveillance des événements",
    EventMonitoringCard: (
        <>
            La surveillance des événements cible des groupes de labels spécifiques. Sélectionnez un groupe de labels
            ci-dessus pour configurer des artefacts d'événement spécifiques ciblant ce groupe.
        </>
    ),
    ...
    "Event Monitoring: Select artifacts to collect from label group ":
        "Surveillance des événements: sélectionner les artefacts à collecter à partir du groupe de libellés ",
    "Artifact Collected": "Artefact collecté",
    "Event Monitoring: Configure artifact parameters for label group":
        "Configurer les paramètres de surveillance des événements",
    ...
    "Server Event Monitoring: Select artifacts to collect on the server":
        "Surveillance des événements du serveur: sélectionner les artefacts à collecter sur le serveur",
    "Server Event Monitoring: Configure artifact parameters for server":
        "Surveillance des événements du serveur: configurer les paramètres d'artefact pour le serveur",
    "Server Event Monitoring: Review new event tables":
        "Surveillance des événements du serveur: vérifier les nouvelles tables d'événements",
    ...
    "You are about to permanently delete the notebook for this hunt":
        "Vous êtes sur le point de supprimer définitivement le carnet de notes de cette chasse",
