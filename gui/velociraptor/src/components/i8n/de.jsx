...
const Deutsch = {
    ...
    "Quarantine description": (
        <>
            ...
            <p>
                Während der Quarantäne kann der Host, außer mit dem Velociraptor-Server, mit keinem andere Netzwerk
                kommunizieren.
            </p>
        </>
    ),
    ...
    "Cannot Quarantine host message": (os_name, quarantine_artifact) => (
        <>
            <Alert variant="warning">
                {quarantine_artifact ? (
                    <p>
                        Diese Velociraptor-Instanz verfügt nicht über das Artefakt <b>{quarantine_artifact}</b>, das
                        erforderlich ist, um Hosts unter Quarantäne zu stellen, auf denen {os_name} ausgeführt wird.
                    </p>
                ) : (
                    <p>
                        Für diese Velociraptor-Instanz ist kein Artefaktname definiert, um Hosts unter Quarantäne zu
                        stellen, auf denen {os_name} ausgeführt wird.
                    </p>
                )}
            </Alert>
        </>
    ...
    "Select artifact pack (Zip file with YAML definitions)":
        "Artefaktpaket auswählen (Zip-Datei mit YAML-Definitionen)",
    ...
    "Set a password in user preferences to lock the download file.":
        "Legen Sie in den Benutzereinstellungen ein Passwort fest, um die Download-Datei zu sichern.",
    ...
    "Please create a hunt that collects one or more of the following artifacts.":
        "Bitte erstellen Sie eine Hunt, die eines oder mehrere der folgenden Artefakte sammelt.",
    Requests: "Anfragen",
    Notebook: "Notizbuch",
    ...
    ArtifactDeletionDialog: (session_id, artifacts, total_bytes, total_rows) => (
        <>
            ...
            <b>{session_id}</b>.
            <br />
            Diese Sammlung hatte die Artefakte <b className="wrapped-text">{artifacts}</b>
            <br />
            <br />
            Wir gehen davon aus, {total_bytes.toFixed(0)} MB Massenspeicher freizugeben data und {total_rows} Zeilen.
        </>
    ),
    "Save this collection to your Favorites": "Speichern Sie diese Sammlung in Ihren Favoriten",
    ArtifactFavorites: (artifacts) => (
        <>
            Sie können die gleiche Sammlung ganz einfach von Ihrem sammeln Favoriten in der Zukunft.
            <br />
    ...
    "No artifacts configured. Please add some artifacts to collect":
        "Keine Artefakte konfiguriert. Bitte fügen Sie einige Artefakte zum Sammelung hinzu",
    ...
    "If set collection will be terminated after this many seconds with no progress.":
        "Wenn die Satzsammlung nach so vielen Sekunden ohne Fortschritt beendet wird.",
    ...
    "Create Offline collector: Select artifacts to collect":
        "Offline-Collector erstellen: Zu sammelnde Artefakte auswählen",
    "Configure Collection": "Collection konfigurieren",
    "Create Offline Collector: Configure artifact parameters":
        "Offline-Collector erstellen: Artefaktparameter konfigurieren",
    ...
    "No hunts exist in the system. You can start a new hunt by clicking the New Hunt button above.":
        'Auf dem System sind keine Hunts vorhanden. Sie können eine neue Hunt starten, indem Sie oben auf die Schaltfläche "Neue Hunt" klicken.',
    ...
    RecursiveVFSMessage: (path) => (
        <>
            ...
            <br />
            <br />
            Dadurch können große Datenmengen vom Client übertragen werden. Das Standard-Upload-Limit beträgt 1 GB, aber
            Sie können es im Bildschirm „Gesammelte Artefakte“ ändern.
        </>
    ),
    ...
    "Refresh this directory (sync its listing with the client)":
        "Dieses Verzeichnis aktualisieren (Eintrag mit dem Client synchronisieren)",
    "Recursively refresh this directory (sync its listing with the client)":
        "Dieses Verzeichnis rekursiv aktualisieren (Eintrag mit dem Client synchronisieren)",
    ...
    "No data available. Refresh directory from client by clicking above.":
        "Keine Daten verfügbar. Aktualisieren Sie das Verzeichnis vom Client, indem Sie oben klicken.",
    "Please select a file or a folder to see its details here.":
        "Bitte wählen Sie eine Datei oder einen Ordner aus, um die Details hier anzuzeigen.",
    ...
    ToolLocalDesc: (
        <>
            Das Tool wird bei Bedarf vom Velociraptor-Server an die Clients ausgeliefert. Der Client speichert das Tool
            auf seiner eigenen Festplatte und vergleicht den Hash, wenn es das nächste Mal benötigt wird. Die Tools
            werden nur heruntergeladen, wenn sich ihr Hash geändert hat.
        </>
    ),
    ServedFromURL: (base_path, url) => (
        <>
            Die Clients rufen das Tool bei Bedarf direkt von
            <a href={base_path + url}>{url}</a> ab. Wenn der Hashwert nicht mit dem erwarteten Hashwert übereinstimmt,
            weisen die Clients die Datei zurück.
        </>
    ),
    ...
    PlaceHolder: (
        <>
            Der Hash-Wert des Tools ist derzeit unbekannt. Das erste Mal, wenn das Tool benötigt wird, lädt Velociraptor
            es von seiner Upstream-URL herunter und berechnet den Hash.
        </>
    ),
    ToolHash: (
        <>
            Der Hashwert des Tools wurde berechnet. Wenn Clients dieses Tool verwenden müssen, stellt es sicher, dass
            dieser Hash mit dem herunter geladenen Binary übereinstimmt.
        </>
    ),
    AdminOverride: (
        <>
            Das Tool wurde manuell von einem Administrator hochgeladen - es wird beim nächsten
            Velociraptor-Server-Update nicht automatisch aktualisiert.
        </>
    ),
    ToolError: (
        <>
            Der Hash des Tools ist nicht bekannt und es ist keine URL definiert. Es wird unmöglich sein, dieses Tools in
            einem Artefakt zu verwenden, da Velociraptor nicht in der Lage ist, es aufzulösen. Sie können eine Datei
            manuell hochladen.
        </>
    ),
    OverrideToolDesc: (
        <>
            Als Administrator können Sie manuell eine Binärdatei hochladen, die als dieses Tool verwendet werden soll.
            Dadurch wird die Upstream-URL-Einstellung außer Kraft gesetzt und Ihr Tool allen Artefakten zur Verfügung
            gestellt, die es benötigen. Alternativ können Sie eine URL festlegen, von der die Clients die Tools abrufen
            können.
        </>
    ),
    ...
    "Select label to edit its event monitoring table":
        "Label auswählen, um seine Event-Monitoringtabelle zu bearbeiten",
    EventMonitoringCard: (
        <>
            Die Event-Monitoring zielt auf bestimmte Labelgruppen ab. Wählen Sie oben eine Kennzeichnungsgruppe aus, um
            spezifische Ereignisartefakte für diese Gruppe zu konfigurieren.
        </>
    ),
    ...
    "Event Monitoring: Select artifacts to collect from label group ":
        "Event-Monitoring: Zu sammelnde Artefakte aus der Labelgruppe auswählen ",
    "Artifact Collected": "Artefakt gesammelt",
    "Event Monitoring: Configure artifact parameters for label group ":
        "Event-Monitoring: Konfigurieren Sie Artefaktparameter für die Labelgruppe ",
    ...
    "Server Event Monitoring: Select artifacts to collect on the server":
        "Server Event-Monitoring: Wählen Sie Artefakte aus, die auf dem Server gesammelt werden sollen",
    "Server Event Monitoring: Configure artifact parameters for server":
        "Server Event-Monitoring: Artefaktparameter für Server konfigurieren",
    ...
    "You are about to permanently delete the notebook for this hunt":
        "Sie sind dabei, das Notizbuch für diese Hunt endgültig zu löschen",
