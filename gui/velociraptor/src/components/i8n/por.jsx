...
const Portuguese = {
    ...
    "Quarantine description": (
        <>
            ...
            <p>
                Durante a quarentena, o host não pode comunicar-se com nenhuma outra rede, exceto com o Servidor
                Velociraptor.
            </p>
        </>
    ),
    ...
    "Cannot Quarantine host message": (os_name, quarantine_artifact) => (
        <>
            <Alert variant="warning">
                {quarantine_artifact ? (
                    <p>
                        Esta instância do Velociraptor não possui o artefato <b>{quarantine_artifact}</b> necessário
                        para colocar em quarentena os hosts {os_name}.
                    </p>
                ) : (
                    <p>
                        Esta instância do Velociraptor não possui um nome de artefato definido para colocar em
                        quarentena os hosts {os_name}.
                    </p>
                )}
            </Alert>
        </>
    ...
    "Set a password in user preferences to lock the download file.":
        "Definir senha para proteger os arquivos baixados.",
    ...
    "Please create a hunt that collects one or more of the following artifacts.":
        "Favor criar uma Investigação que colete um ou mais dos seguintes artefatos.",
    ...
    ArtifactDeletionDialog: (session_id, artifacts, total_bytes, total_rows) => (
        <>
            ...
            <b>{session_id}</b>.
            <br />
            Esta coleta tinha os artefatos <b className="wrapped-text">{artifacts}</b>
            <br />
            <br />
            Calculado liberar {total_bytes.toFixed(0)} MB de armazenamento dados e {total_rows} linhas.
        </>
    ),
    "Save this collection to your Favorites": "Salvar esta coleta em seus Favoritos",
    ArtifactFavorites: (artifacts) => (
        <>
            Você pode facilmente reutilizar a mesma coleta salva em seus favoritos no futuro.
            <br />
    ...
    "No artifacts configured. Please add some artifacts to collect":
        "Nenhum artefato configurado. Por favor, adicione alguns artefatos para coletar",
    ...
    "If set collection will be terminated after this many seconds with no progress.":
        "Caso configurado, a coleta terminará após se passar o numero selecionado em segundos sem nenhum progresso.",
    ...
    "Create Offline Collector: Configure artifact parameters":
        "Criar Coletor Offline: Configurar Parâmetros de Artefato",
    ...
    "No hunts exist in the system. You can start a new hunt by clicking the New Hunt button above.":
        "Nenhuma Investigação existe no sistema. Você pode iniciar uma Investigação clicando no botão Nova Investigação acima",
    ...
    RecursiveVFSMessage: (path) => (
        <>
            ...
            <br />
            <br />
            Isso permite que grandes quantidades de dados sejam transferidas do terminal. O limite de upload padrão é de
            1 GB, mas você pode alterá-lo na tela Artefatos Colectados.
        </>
    ),
    ...
    "Refresh this directory (sync its listing with the client)":
        "Atualizar este diretório (sincronizar lista com o cliente)",
    "Recursively refresh this directory (sync its listing with the client)":
        "Atualizar recursivamente este diretório (sincronizar lista com o cliente)",
    ...
    "No data available. Refresh directory from client by clicking above.":
        "Nenhum dado disponível. Atualize o diretório do cliente clicando acima.",
    "Please select a file or a folder to see its details here.":
        "Selecione um arquivo ou pasta para ver seus detalhes aqui.",
    ...
    ToolLocalDesc: (
        <>
            A ferramenta é fornecida pelo Servidor Velociraptor, se aplicável. O cliente armazena a ferramenta em cache
            em seu próprio disco e depois compara seu hash quando necessário. As ferramentas só serão baixadas se o hash
            mudar.
        </>
    ),
    ServedFromURL: (base_path, url) => (
        <>
            ...
            <a href={base_path + url}>{url}</a> se necessário. Observe que, se o hash não corresponder ao hash esperado,
            os clientes rejeitarão o arquivo.
        </>
    ),
    ServedFromGithub: (github_project, github_asset_regex) => (
        <>
            O URL da ferramenta é atualizado por GitHub como a versão mais recente do projeto
    ...
    PlaceHolder: (
        <>
            O hash da ferramenta é atualmente desconhecido. Na primeira vez que a ferramenta for requisitada, o
            Velociraptor irá baixá-la da URL indicada e calculará seu hash.
        </>
    ),
    ToolHash: (
        <>
            O hash da ferramenta foi calculado. Se os clientes precisarem usar, eles usarão este hash para garantir que
            a ferramenta corresponde àquela que estão fazendo Download.
        </>
    ),
    AdminOverride: (
        <>
            A ferramenta foi carregada manualmente por um Administrador - não é atualizada automaticamente no próxima
            atualização do Servidor Velociraptor.
        </>
    ),
    ToolError: (
        <>
            O hash da ferramenta é desconhecido e não há um URL definido. Será impossível usar esta ferramenta em um
            Artefato porque o Velociraptor não poderá materializá-la. Você pode carregar um arquivo manualmente.
        </>
    ),
    OverrideToolDesc: (
        <>
            Como administrador, você pode carregar manualmente um arquivo binário para utilizá-lo como ferramenta. Isso
            sobrescreve o configuração de URL upstream e disponibiliza sua ferramenta para todos Artefatos que precisam.
            Como alternativa, defina uma URL conhecida para clientes obterem ferramentas.
        </>
    ),
    ...
    "Select label to edit its event monitoring table":
        "Selecione o rótulo para editar sua tabela de monitoramento de eventos",
    EventMonitoringCard: (
        <>
            O monitoramento de eventos visa grupos de rótulos específicos. Selecione um grupo de rótulos acima para
            configurar especificamente Artefatos de eventos direcionados a este grupo.
        </>
    ),
    ...
    "Event Monitoring: Select artifacts to collect from label group ":
        "Monitoramento de eventos: Selecione artefatos para coletar do grupo de rótulos ",
    "Artifact Collected": "Artefato coletado",
    "Event Monitoring: Configure artifact parameters for label group ":
        "Monitoramento de eventos: configurar parâmetros de artefato para o grupo de rótulos",
    ...
    "Server Event Monitoring: Select artifacts to collect on the server":
        "Monitoramento de Eventos do Servidor: Selecionar artefatos para colectar no servidor.",
    "Server Event Monitoring: Configure artifact parameters for server":
        "Monitoramento de Eventos do Servidor: Configurar parâmetros de artefato para o servidor",
    "Server Event Monitoring: Review new event tables":
        "Monitoramento de Eventos do Servidor: Revisar novas tabelas de eventos",
    ...
    "You are about to permanently delete the notebook for this hunt":
        "Você está prestes a excluir permanentemente o Notebook para esta investigação",
