...
class ParameterSuggestion extends React.Component {
    ...
    getSuggestions = (value) => {
        ...
        return inputLength === 0
            ? []
            : this.props.parameters.filter((x) => {
                  try {
    ...
    renderSuggestion = (suggestion) => <div key={suggestion.name}>{suggestion.name}</div>;
...
export default class NewCollectionConfigParameters extends React.Component {
    ...
    artifactParameterRenderer = (artifact) => {
        let artifact_parameters = (this.props.parameters && this.props.parameters[artifact.name]) || {};
        ...
        let form_parameters = _.map(artifact.parameters, (param, idx) => {
            ...
            return (
                <VeloForm
                    ...
                    name={idx}
                    value={value}
                    setValue={(value) => this.setValue(artifact.name, param.name, value)}
                />
            );
        });
                ...
                return (
                    <ButtonGroup>
                        ...
                        <Button
                            ...
                            onClick={() => this.props.setArtifacts(remove_artifact(this.props.artifacts, rowKey))}
                        >
                ...
                <Modal.Body className="new-collection-parameter-page selectable">
                    {!_.isEmpty(this.props.artifacts) ? (
                        <BootstrapTable
                            ...
                            expandRow={expandRow}
                            columns={[
                                { dataField: "name", text: T("Artifact") },
