...
export default class FlowOverview extends React.Component {
    ...
    componentDidMount = () => {
        ...
        let lock_password = this.context.traits && this.context.traits.default_password;
    ...
    prepareDownload = (opts) => {
        let options = opts || {};
        if (this.state.lock) {
            options.password = this.context.traits && this.context.traits.default_password;
        }
        ...
        api.post("v1/CreateDownload", options, this.source.token).then((response) => {
            this.getDetailedFlow();
        });
    };
    ...
    getDetailedFlow = () => {
        ...
        api.get(
            ...
            let available_downloads = response.data.available_downloads && response.data.available_downloads.files;
                            ...
                            <dl className="row">
                                ...
                                <dd className="col-8">
                                    {_.map(artifacts, function (v, idx) {
                                        return (
                                            <ArtifactLink artifact={v} key={idx}>
                                                {v}
                                            </ArtifactLink>
                                        );
                                    })}
                                ...
                                <dd className="col-8">
                                    {flow.execution_duration
                                        ? (flow.execution_duration / 1000000000).toFixed(2) + " " + T("seconds")
                                ...
                                <dd className="col-8">
                                    {" "}
                                    {flow.request.timeout || "600"} {T("seconds")}
                                </dd>
                                <dt className="col-4">{T("Max Rows")}</dt>
                                <dd className="col-8">
                                    {" "}
                                    {flow.request.max_rows || "1m"} {T("rows")}
                                </dd>
                                <dt className="col-4">{T("Max Mb")}</dt>
                                <dd className="col-8">
                                    {" "}
                                    {((flow.request.max_upload_bytes || 1048576000) / 1024 / 1024).toFixed(2)} Mb
                                </dd>
                                <br />
                            </dl>
                    ...
                    <Card>
                        ...
                        <Card.Body>
                            <dl className="row">
                                ...
                                <dd className="col-8">{flow.total_collected_rows || 0}</dd>

                                <dt className="col-4">{T("Uploaded Bytes")}</dt>
                                <dd className="col-8">
                                    {flow.total_uploaded_bytes || 0} / {flow.total_expected_uploaded_bytes || 0}
                                </dd>

                                <dt className="col-4">{T("Files uploaded")}</dt>
                                <dd className="col-8">{uploaded_files.length || flow.total_uploaded_files || 0}</dd>
                                        ...
                                        {this.state.expand_sparse ? (
                                            ...
                                            >
                                                <span className="d-inline-block">
                                                    <Button
                                                        onClick={() =>
                                                            this.setState({ expand_sparse: !this.state.expand_sparse })
                                                        }
                                                        variant="default"
                                                    >
