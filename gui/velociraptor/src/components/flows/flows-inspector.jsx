...
class FlowInspector extends React.Component {
    ...
    setDefaultTab = (tab) => {
        ...
        this.props.history.push(
            "/collected/" + this.props.flow.client_id + "/" + this.props.flow.session_id + "/" + tab
        );
    };
            ...
            <div className="padded">
                <Tabs activeKey={this.state.tab} onSelect={this.setDefaultTab}>
                    <Tab eventKey="overview" title={T("Artifact Collection")}>
                        {this.state.tab === "overview" && <FlowOverview flow={this.state.detailed_flow.context} />}
                    </Tab>
                    <Tab eventKey="uploads" title={T("Uploaded Files")}>
                        {this.state.tab === "uploads" && <FlowUploads flow={this.props.flow} />}
                    </Tab>
                    <Tab eventKey="requests" title={T("Requests")}>
                        {this.state.tab === "requests" && <FlowRequests flow={this.props.flow} />}
                    </Tab>
                    <Tab eventKey="results" title={T("Results")}>
                        {this.state.tab === "results" && <FlowResults flow={this.state.detailed_flow.context} />}
                    </Tab>
                    <Tab eventKey="logs" title={T("Log")}>
                        {this.state.tab === "logs" && <FlowLogs flow={this.state.detailed_flow.context} />}
                    </Tab>
                    <Tab eventKey="notebook" title={T("Notebook")}>
                        {this.state.tab === "notebook" && <FlowNotebook flow={this.props.flow} />}
                    </Tab>
                </Tabs>
            </div>
        );
