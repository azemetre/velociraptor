                            ...
                            <Form.Group as={Row}>
                                ...
                                <Col sm="8">
                                    <Form.Control
                                        as="textarea"
                                        placeholder={T(
                                            "Public Key/Certificate To Encrypt With. If X509, Defaults To Frontend Cert"
                                        )}
                                        spellCheck="false"
...
export default class OfflineCollectorWizard extends React.Component {
    ...
    prepareRequest = () => {
        ...
        env.push({ key: "OS", value: this.state.collector_parameters.target_os });
        env.push({ key: "artifacts", value: JSON.stringify(_.map(this.state.artifacts, (item) => item.name)) });
        ...
        env.push({ key: "target_args", value: JSON.stringify(this.state.collector_parameters.target_args) });
        ...
        env.push({ key: "opt_progress_timeout", value: JSON.stringify(this.state.resources.progress_timeout) });
        env.push({ key: "opt_timeout", value: JSON.stringify(this.state.resources.timeout) });
        env.push({ key: "opt_cpu_limit", value: JSON.stringify(this.state.resources.cpu_limit) });
    ...
    render() {
        ...
        let extra_tools = [];
        if (this.state.collector_parameters && this.state.collector_parameters.target_os) {
                    ...
                    <ObserveKeys>
                        <StepWizard ref={(n) => (this.step = n)}>
                            ...
                            <OfflineCollectionResources
                                ...
                                paginator={
                                    new OfflinePaginator("Specify Resources", "New Collection: Specify Resources")
                                }
                                setResources={this.setResources}
                            />

                            <NewCollectionRequest
                                paginator={new OfflinePaginator("Review", "Create Offline Collector: Review request")}
                            ...
                            <NewCollectionLaunch
                                artifacts={this.state.artifacts}
                                tools={extra_tools}
                                paginator={new OfflinePaginator("Launch", "Create Offline Collector: Create collector")}
                                launch={this.launch}
                            />
                        </StepWizard>
                    </ObserveKeys>
