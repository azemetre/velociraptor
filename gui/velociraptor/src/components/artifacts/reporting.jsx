...
import {
    NotebookLineChart,
    NotebookTimeChart,
    NotebookScatterChart,
    NotebookBarChart,
} from "../notebooks/notebook-chart-renderer.jsx";

// Renders a report in the DOM.

const parse_param = (domNode) => JSON.parse(decodeURIComponent(domNode.attribs.params || "{}"));
...
export default class VeloReportViewer extends React.Component {
    ...
    updateReport() {
        ...
        api.post("v1/GetReport", params, this.source.token)
            .then((response) => {
            ...
            replace: (domNode) => {
                if (domNode.name === "inline-table-viewer") {
                    try {
                        ...
                        return <VeloTable rows={rows} columns={response.Columns} />;
                    } catch (e) {}
                }
                ...
                if (domNode.name === "grr-csv-viewer") {
                    ...
                    if (value) {
                        ...
                        return <VeloTable rows={rows} columns={data.Columns} />;
                    }

                    try {
                        return <NotebookTableRenderer refresh={this.props.refresh} params={parse_param(domNode)} />;
                    } catch (e) {
                        return domNode;
                    }
                }
                ...
                if (domNode.name === "grr-tool-viewer") {
                    ...
                    return <ToolViewer name={name} version={this.state.version} />;
                }
                ...
                if (domNode.name === "grr-line-chart") {
                    ...
                    try {
                        ...
                        return <VeloLineChart data={rows} columns={data.Columns} params={params} />;
                    } catch (e) {
                        return domNode;
                    }
                }
                ...
                if (domNode.name === "time-chart") {
                    ...
                    try {
                        let rows = JSON.parse(data.Response);
                        let params = JSON.parse(decodeURIComponent(domNode.attribs.params));
                        return <VeloTimeChart data={rows} columns={data.Columns} params={params} />;
                    } catch (e) {
                        return domNode;
                    }
                }
