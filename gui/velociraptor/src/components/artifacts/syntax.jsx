...
export default class Completer {
    ...
    guessContext = (previous, prefix) => {
        ...
        if (match) {
            results.push({ context: "plugin_args", pos: match.index + match[1].length, name: match[2] });
        }
        ...
        if (match) {
            results.push({ context: "function_args", pos: match.index + match[1].length, name: match[1] });
        }
        ...
        for (var i = 0; i < this.state.completions.length; i++) {
            ...
            if (item.type === "Plugin") {
                ...
                completions.push({
                    ...
                    value: item.name,
                    meta: item.type,
                    docHTML: '<div class="arg-help">' + escapeHTML(item.description) + "</div>",
                });
            }
        }
        return completions;
            ...
            if ((item.type === "Plugin" || item.type === "Artifact") && item.name === name) {
                ...
                for (var j = 0; j < arg_desc.length; j++) {
                    ...
                    completions.push({
                        ...
                        score: 1000,
                        meta: meta,
                        docHTML: '<div class="arg-help">' + escapeHTML(arg.description) + "</div>",
                    });
                }
            }
        }
    ...
    getFunctionCompletions = (prefix) => {
        ...
        for (var i = 0; i < this.state.completions.length; i++) {
            ...
            if (item.type === "Function") {
                ...
                var html = "";
                if (item.description) {
                    html = '<div class="arg-help">' + escapeHTML(item.description) + "</div>";
                }
    ...
    initializeAceEditor = (ace, options) => {
        ...
        var vqlCompleter = {
            ...
            getCompletions: (editor, session, pos, prefix, callback) => {
                ...
                previous_rows[last_idx] = previous_rows[last_idx].slice(0, pos.column - prefix.length);
                ...
                if (context.context === "string") {
                    ...
                    callback(null, this.getPluginCompletions(prefix).concat(this.getArtifactCompletions(prefix)));
                } else if (context.context === "plugin_args") {
                    callback(
                        null,
                        this.getPluginArgsCompletions(context.name, prefix).concat(this.getFunctionCompletions(prefix))
                    );
                } else if (context.context === "function_args") {
                    callback(
                        null,
                        this.getFunctionArgsCompletions(context.name, prefix).concat(
                            this.getFunctionCompletions(prefix)
                        )
                    );
                } else {
                    callback(null, this.getKeywordCompletions(prefix).concat(this.getFunctionCompletions(prefix)));
                }
            },
        };
