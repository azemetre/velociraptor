...
export default class ArtifactsUpload extends React.Component {
    ...
    uploadFile = () => {
        ...
        reader.onload = (event) => {
            ...
            api.post("v1/LoadArtifactPack", request, this.source.token).then((response) => {
                let uploaded = _.map(response.data.successful_artifacts, (x, idx) => {
    ...
    render() {
        let columns = formatColumns([{ dataField: "name", text: T("Artifact Name"), sort: true, filtered: true }]);
                            ...
                            <Form className="selectable">
                                <InputGroup className="mb-3">
                                    <InputGroup.Prepend>
                                        ...
                                        >
                                            {this.state.loading ? <FontAwesomeIcon icon="spinner" spin /> : "Upload"}
                                        </InputGroup.Text>
                                    </InputGroup.Prepend>
                                    <Form.File
