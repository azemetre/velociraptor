...
export default class NewArtifactDialog extends React.Component {
    ...
    fetchArtifact = () => {
        if (!this.state.initialized_from_parent) {
            ...
            api.get("v1/GetArtifactFile", { name: this.props.name }, this.source.token).then((response) => {
                this.setState({
        ...
        return (
            ...
            >
                <Modal.Header closeButton onHide={this.props.onClose}>
                    <Modal.Title>
                        {this.props.name ? T("Edit Artifact", this.props.name) : T("Create a new artifact")}
                    </Modal.Title>
                </Modal.Header>
                <Modal.Body>
                    <VeloAce
