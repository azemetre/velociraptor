...
export class AddTimelineDialog extends React.Component {
    ...
    addTimeline = () => {
        if (this.state.timeline) {
            ...
            this.props.addCell('{{ Scope "Timeline" | Timeline }}', "Markdown", [
    ...
    render() {
        ...
        return (
            <Modal show={true} size="sm" onHide={this.props.closeDialog}>
                ...
                <Modal.Footer>
                    ...
                    <Button variant="primary" onClick={this.addTimeline}>
                        Submit
                    </Button>
                </Modal.Footer>
            </Modal>
    ...
    addTimeline = () => {
        ...
        api.post(
            "v1/CollectArtifact",
            {
                ...
                specs: [
                    {
                        artifact: "Server.Utils.AddTimeline",
                        parameters: {
                            env: [
            ...
            <Modal show={true} size="lg" onHide={this.props.closeDialog}>
                ...
                <Modal.Footer>
                    ...
                    <Button
                        variant="primary"
                        disabled={
                            !this.state.timeline || !this.state.time_column || !this.state.name || this.state.loading
                        }
                        onClick={this.addTimeline}
                    >
                        {this.state.loading ? (
                            <>
                                <FontAwesomeIcon icon="spinner" spin /> {T("Running")}{" "}
                            </>
                        ) : (
                            <> {T("Add Timeline")} </>
                        )}
