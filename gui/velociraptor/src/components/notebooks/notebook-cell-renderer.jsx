...
export default class NotebookCellRenderer extends React.Component {
    ...
    componentDidUpdate = (prevProps, prevState, rootNode) => {
        ...
        if (props_cell_timestamp !== this.state.cell_timestamp || props_cell_id !== current_cell_id) {
            // Prevent further updates to this cell by setting the
            // cell id and timestamp.
            this.setState({ cell_timestamp: props_cell_timestamp, loading: true, cell_id: props_cell_id });
            this.fetchCellContents();
        }
    };
        ...
        for (var i = 0; i < items.length; i++) {
            ...
            if (item.kind === "file") {
                ...
                reader.onload = (event) => {
                    ...
                    api.post("v1/UploadNotebookAttachment", request, this.source.token).then(
                        (response) => {
    ...
    showSuggestions = () => {
        let suggestions = this.props.notebook_metadata && this.props.notebook_metadata.suggestions;
        if (!suggestions) {
            return <></>;
        }
                    ...
                    {_.map(suggestions, (x) => {
                        return (
                            <Dropdown.Item
                                key={x.name}
                                onClick={() => {
                                    this.props.addCell(this.state.cell.cell_id, x.type, x.input, x.env);
                                }}
                                title="{x.name}"
                            >
                                {x.name}
        ...
        let non_editing_toolbar = (
            <>
                <ButtonGroup>
                    ...
                    <Button
                        data-tooltip={T("Stop Calculating")}
                        data-position="right"
                        className="btn-tooltip"
                        disabled={!this.state.cell.calculating}
                    ...
                    >
                        ...
                        <Dropdown.Menu>
                            ...
                            <Dropdown title={T("Suggestion")} drop="right" variant="default-outline">
                                <Dropdown.Toggle className="dropdown-item" variant="default-outline">
                                    {T("Suggestions")}
                                </Dropdown.Toggle>
                            ...
                            <Dropdown.Item title={T("Add Cell From This Cell")} onClick={this.addCellFromCell}>
                ...
                <ButtonGroup className="float-right">
                    ...
                    >
                        ...
                        {this.state.cell.duration && <span>&nbsp;({this.state.cell.duration}s) </span>}
                    </Button>
                </ButtonGroup>
            </>
        );
            ...
            <>
                <ButtonGroup>
                    ...
                    {this.state.cell.type === "vql" && (
                        <Button
                            data-tooltip={T("Reformat Format VQL")}
                            data-position="right"
                            className="btn-tooltip"
                ...
                <ButtonGroup className="float-right">
                    ...
                    >
                        {_.map(cell_types, (v, idx) => {
                            return (
                                <option value={v.toLowerCase()} key={idx}>
                                    {v}
                                </option>
                            );
                        })}
        ...
        return (
            <>
                ...
                <div className={classNames({ selected: selected, "notebook-cell": true })}>
                    ...
                    <div
                        className={classNames({ collapsed: this.state.collapsed, "notebook-output": true })}
                        onClick={() => {
                    ...
                    >
                        ...
                        {selected && (
                            <>
                                ...
                                <Button variant="secondary" onClick={() => this.setState({ showMoreLogs: true })}>
                                    {T("Logs")}
                                </Button>
                            </>
                        )}
