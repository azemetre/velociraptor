...
export default class VeloClientSummary extends Component {
    ...
    getClientInfo = () => {
        ...
        if (client_id) {
            api.get("v1/GetClient/" + client_id, params, this.source.token)
                .then((response) => {
    ...
    timeDifference(last_seen_at) {
        ...
        var differenceSec = Math.abs(Math.round((currentTimeMs - inputTimeMs) / 1000));
    ...
    render() {
        ...
        return (
            <div className="client-summary client-status toolbar-buttons">
                <Link className="nav-link client-name" to={"/host/" + this.props.client.client_id}>
                    {fqdn}
                </Link>
