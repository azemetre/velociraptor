...
class _VeloShellCell extends Component {
    ...
    viewFlow = (target) => {
        if (!this.props.flow || !this.props.flow.session_id || !this.props.flow.client_id) {
            return;
        }

        let client_id = this.props.flow.client_id;
        let session_id = this.props.flow.session_id;
        this.props.history.push("/collected/" + client_id + "/" + session_id + "/" + target);
    };

    // Retrieve the flow result from the server and show it.
    loadData = () => {
        if (!this.props.flow || !this.props.flow.request || !this.props.flow.request.artifacts) {
            return;
        }
    ...
    render() {
        ...
        if (this.state.collapsed) {
            buttons.push(
                <button
                    className="btn-tooltip btn btn-default"
                    key={1}
        ...
        if (this.props.flow.state === "RUNNING") {
            ...
            flow_status.push(
                <button className="btn btn-outline-info" key={6} disabled>
                    <i>
            ...
            flow_status.push(
                <button className="btn btn-outline-info" key={7} disabled>
            ...
            flow_status.push(
                <button className="btn btn-outline-info" key={8} disabled>
                    <i>
        ...
        if (this.state.loaded) {
            output = [
                this.state.output.map((item, index) => {
                    return (
                        <div className="notebook-output" key={index}>
                            <pre> {item.Stdout} </pre>
                        </div>
                    );
                }),
...
class _VeloVQLCell extends Component {
    ...
    render() {
        ...
        if (this.props.flow.state === "RUNNING") {
            ...
            flow_status.push(
                <button className="btn btn-outline-info" key={15} disabled>
                    <i>
                        <FontAwesomeIcon icon="spinner" spin />
                    </i>
                    <VeloTimestamp usec={this.props.flow.create_time / 1000} />
            ...
            flow_status.push(
                <button className="btn btn-outline-info" key={12} disabled>
                    <VeloTimestamp usec={this.props.flow.active_time / 1000} />
                    by {this.props.flow.request.creator}
                </button>
            );
            ...
            flow_status.push(
                <button className="btn btn-outline-info" key={13} disabled>
                    <i>
                        <FontAwesomeIcon icon="exclamation" />
                    </i>
                    <VeloTimestamp usec={this.props.flow.create_time / 1000} />
        ...
        if (this.state.loaded) {
            let artifact =
                ...
                this.props.flow.request.artifacts[0];
            let params = {
                artifact: artifact,
                client_id: this.props.flow.client_id,
                flow_id: this.props.flow.session_id,
            ...
            <>
                ...
                >
                    <div className="notebook-input">
                        ...
                        <VeloAce text={this.getInput()} mode="sql" aceConfig={this.aceConfig} />
                    </div>
                    {output}
                </div>
            </>
...
class ShellViewer extends Component {
    ...
    renderCells(flows) {
        return flows.map((flow, index) => {
            let artifact = flow && flow.request && flow.request.artifacts && flow.request.artifacts[0];
    ...
    render() {
        ...
        return (
            <>
                ...
                <div className="shell-results">{this.renderCells(this.state.flows)}</div>
            </>
        );
    }
}
