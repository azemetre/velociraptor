    ...
    renderUnavailable() {
        ...
        return (
            <Modal show={true} onHide={this.props.onClose}>
                ...
                <Modal.Body>{T("Cannot Quarantine host message", os_name, this.state.quarantine_artifact)}</Modal.Body>
                <Modal.Footer>
...
class VeloHostInfo extends Component {
    ...
    updateClientInfo = () => {
        ...
        let client_id = this.props.client && this.props.client.client_id;
        if (client_id) {
            api.get("v1/GetClient/" + client_id, params, this.source.token).then((response) => {
    ...
    fetchMetadata = () => {
        ...
        api.get("v1/GetClientMetadata/" + this.props.client.client_id, {}, this.source.token).then((response) => {
            if (response.cancel) return;
            ...
            this.setState({ metadata: metadata, metadata_loading: false });
        });
    };
                    ...
                    <Card>
                        ...
                        <Card.Body>
                            <dl className="row">
                                ...
                                <dd className="col-sm-9">{info.client_id}</dd>

                                <dt className="col-sm-3">{T("Agent Version")}</dt>
                                <dd className="col-sm-9">
                                    {info.agent_information && info.agent_information.version}{" "}
                                </dd>

                                <dt className="col-sm-3">{T("Agent Build Time")}</dt>
                                <dd className="col-sm-9">
                                    <VeloTimestamp usec={info.agent_information && info.agent_information.build_time} />
                                </dd>
                                ...
                                <dd className="col-sm-9">{info.last_ip}</dd>
                            ...
                            <dl className="row">
                                ...
                                <dd className="col-sm-9">{info.os_info.system}</dd>

                                <dt className="col-sm-3">{T("Hostname")}</dt>
                                <dd className="col-sm-9">{info.os_info.hostname}</dd>

                                <dt className="col-sm-3">{T("FQDN")}</dt>
                                <dd className="col-sm-9">{info.os_info.fqdn}</dd>

                                <dt className="col-sm-3">{T("Release")}</dt>
                                <dd className="col-sm-9">{info.os_info.release}</dd>

                                <dt className="col-sm-3">{T("Architecture")}</dt>
                                <dd className="col-sm-9">{info.os_info.machine}</dd>
                            </dl>
                            <hr />
                            <VeloForm
        ...
        if (this.state.mode === "detailed") {
            return (
                <div className="client-details dashboard">
                    <VeloReportViewer
                        artifact={this.props.client.last_interrogate_artifact_name || "Generic.Client.Info"}
    ...
    renderQuarantine = () => {
        if (this.context.traits && this.context.traits.disable_quarantine_button) {
            return;
        }
        let info = this.getClientInfo();
                ...
                <div className="full-width-height">
                    <div className="client-info">
                        <div className="btn-group float-left toolbar" data-toggle="buttons">
                            ...
                            <Link to={"/vfs/" + client_id + "/"} role="button" className="btn btn-default">
                                <i>
                                    <FontAwesomeIcon icon="folder-open" />
                                </i>
                                <span className="button-label">{T("VFS")}</span>
                            </Link>
                            <Link to={"/collected/" + client_id} role="button" className="btn btn-default">
                                <i>
                        ...
                        <ToggleButtonGroup
                            ...
                            className="mb-2"
                        >
                            <ToggleButton variant="default" value="brief">
                                <FontAwesomeIcon icon="laptop" />
                                <span className="button-label">{T("Overview")}</span>
                            </ToggleButton>
                            <ToggleButton variant="default" value="detailed">
                                <FontAwesomeIcon icon="tasks" />
                                <span className="button-label">{T("VQL Drilldown")}</span>
                            </ToggleButton>
                            <ToggleButton variant="default" value="shell">
